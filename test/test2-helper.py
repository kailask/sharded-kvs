...
nodes = [
    {},
    {
        "addr": "10.10.1.0:13800",
        "port": 13801,
    },
    {
        "addr": "10.10.2.0:13800",
        "port": 13802,
    },
    {
        "addr": "10.10.3.0:13800",
        "port": 13803,
    },
]

# windows toolbox users will again want to make this the docker machine's ip adress
localhost = "localhost"
...
class Client():
    ...
    def formatResult(self, result):
        ...
        if result != None:
            jsonKeys = ["message", "replaced", "error", "doesExist",
                        "value", "key-count", "shards", "address"]
...
class TestHW3(unittest.TestCase):
    def check_node_id(self, result, node_id, num):
        if "address" in result:  # the receiving node does not store the key
            ...
            # node_id should not equal to the receiving node
            self.assertNotEqual(node_id_, node_id)
    ...
    def test_2(self):
        ...
        for i in range(keys):
            ...
            result = client.putKey(
                "test_2_%d" % i, "a friendly string %d" % i, nodes[id]["port"])
            self.assertEqual_helper(result, addResponse_Success)
        ...
        # view-change
        result = client.viewChange(
            "10.10.1.0:13800,10.10.2.0:13800,10.10.3.0:13800", nodes[1]["port"])
